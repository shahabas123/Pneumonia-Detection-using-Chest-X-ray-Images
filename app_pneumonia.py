...
@st.cache_resource  # Cache the model download and loading
def load_pneumonia_model():
    # Model URL from your GitHub Releases
    model_url = "https://github.com/shahabas123/Pneumonia-Detection-using-Chest-X-ray-Images/releases/download/v1/pneumonia_model.h5"

    try:
        # Download the model if not already present
        if not os.path.exists("pneumonia_model.h5"):
            st.info("Downloading pneumonia detection model... (This may take a minute)")
            response = requests.get(model_url)
            response.raise_for_status()  # Check for download errors

            with open("pneumonia_model.h5", "wb") as f:
